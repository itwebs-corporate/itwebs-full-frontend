...
import { ServiceStages as ServiceStagesType } from '@/shared/types/service-dto-types';

import ServiceStagesCard from './service-stages-card';

export default function ServiceStages({ stages }: { stages: ServiceStagesType[] | [] }) {
  ...
  useEffect(() => {
    ...
    const recalculateLayout = () => {
      ...
      const totalGapWidth = cards.length > 1 ? gapValue * (cards.length - 1) : 0;
      const singleRowWidth = cardsWidth + totalGapWidth;
      const nextValue = singleRowWidth > viewport.clientWidth + 1;

      setIsCarouselEnabled(nextValue);

      if (!nextValue && viewport.scrollLeft !== 0) {
  ...
  const handlerScroll = (direction: 'prev' | 'next') => {
    const viewport = ref.current;
    const list = listRef.current;
    if (!viewport || !list || !isCarouselEnabled) return;
    ...
    <Section className="gap-[48px]">
      ...
      {stages.length > 0 ? (
        <div className="relative w-full">
          ...
          <div
            className={cn(
              'flex w-full max-w-full min-w-0 justify-center',
              isCarouselEnabled
                ? 'no-scrollbar overflow-x-auto overflow-y-hidden sm:justify-start sm:pb-2'
                : 'overflow-visible'
            )}
            ref={ref}
          >
            <ul
              className={cn(
                ...
                isCarouselEnabled
                  ? 'w-max flex-col flex-nowrap sm:flex-row'
                  : 'w-full flex-col sm:flex-row sm:flex-wrap sm:justify-center'
              )}
              ref={listRef}
            >
