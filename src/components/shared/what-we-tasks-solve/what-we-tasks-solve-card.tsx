...
export default function WhatWeTasksSolveCard({
  ...
  return (
    <div className="flex min-h-[278px] w-[85vw] max-w-[420px] shrink-0 snap-center flex-col items-start rounded-[12px] bg-white/10 p-[20px] sm:grid sm:min-h-[142px] sm:w-full sm:max-w-none sm:grid-cols-3 sm:items-center sm:gap-[clamp(16px,2vw,24px)] sm:rounded-[24px] sm:py-3 sm:pr-[clamp(24px,2vw,76px)] sm:pl-[clamp(16px,2vw,32px)] xl:gap-0">
      <div
        className={cn(
          ...
          index === 1 ? 'sm:text-primary sm:bg-secondary sm:border-none' : ''
        )}
      >
        Решение {index < 10 && '0'}
        {index}
